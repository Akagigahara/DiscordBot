...
namespace DiscordBot.Resources.Commands
{
	internal class Create : CommandBase
	{
		...
		{
			string ChannelID = (from option in ((from option in data.options where option.name == "role_select" select option)as CommandOptionData)!.options where option.name == "channel" select option).First().value!;
